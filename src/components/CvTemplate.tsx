...
export default function CvTemplate({ template, profile, className = '', style }: CvTemplateProps) {
  ...
  const headingStyle: React.CSSProperties = { fontSize: '12px', textTransform: 'uppercase', letterSpacing: '1px', color: '#BC181C', fontWeight: 700, marginBottom: '8px', marginTop: '20px' }
  const textStyle: React.CSSProperties = { fontSize: '11px', color: '#374151' }

  if (template === 'classic') {
    return (
      <div className={`cv-template ${className}`} style={{ ...base, ...style }}>
        <div style={{ borderBottom: '2px solid #BC181C', paddingBottom: '16px' }}>
        ...
        <section>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
            {competencias.length ? competencias.map((c, i) => <span key={i} style={{ background: '#eff6ff', color: '#BC181C', fontSize: '10px', padding: '3px 8px', borderRadius: '12px' }}>{c}</span>) : <span style={textStyle}>Nenhuma competência registada.</span>}
          </div>
        </section>
      </div>
    )
  ...
  if (template === 'modern') {
    return (
      <div className={`cv-template ${className}`} style={{ ...base, padding: 0, ...style }}>
        <div style={{ background: 'linear-gradient(135deg, #BC181C 0%, #ECA61B 100%)', color: '#fff', padding: '48px' }}>
        ...
        <div style={{ display: 'flex' }}>
          <div style={{ width: '30%', background: '#f3f4f6', padding: '32px 24px', minHeight: '850px' }}>
            <div style={{ ...headingStyle, color: '#ECA61B' }}>Contactos</div>
            ...
            <div style={{ ...headingStyle, color: '#ECA61B' }}>Competências</div>
            {competencias.map((c, i) => <div key={i} style={{ ...textStyle, marginBottom: '4px' }}>• {c}</div>)}

            <div style={{ ...headingStyle, color: '#ECA61B' }}>Educação</div>
  ...
  if (template === 'minimal') {
    return (
      <div className={`cv-template ${className}`} style={{ ...base, textAlign: 'center', ...style }}>
        ...
        {profile.bio && (
          <section>
            <div style={{ ...headingStyle, textAlign: 'left', borderBottom: '2px solid #BC181C', display: 'inline-block', width: '100%' }}>Perfil</div>
            <p style={{ ...textStyle, textAlign: 'left' }}>{profile.bio}</p>
          </section>
        )}

        <section>
          <div style={{ ...headingStyle, textAlign: 'left', borderBottom: '2px solid #BC181C', display: 'inline-block', width: '100%' }}>Experiência Profissional</div>
        ...
        <section>
          <div style={{ ...headingStyle, textAlign: 'left', borderBottom: '2px solid #BC181C', display: 'inline-block', width: '100%' }}>Educação</div>
          <p style={{ ...textStyle, textAlign: 'left' }}>{profile.nivel_academico || 'Nível académico não informado'}</p>
        </section>

        <section>
          <div style={{ ...headingStyle, textAlign: 'left', borderBottom: '2px solid #BC181C', display: 'inline-block', width: '100%' }}>Competências</div>
          <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'flex-start', gap: '8px' }}>
            {competencias.length ? competencias.map((c, i) => <span key={i} style={{ border: '1px solid #BC181C', color: '#BC181C', fontSize: '10px', padding: '3px 8px', borderRadius: '12px' }}>{c}</span>) : <span style={{ ...textStyle, textAlign: 'left' }}>Nenhuma competência registada.</span>}
          </div>
        </section>
      </div>
    )
  ...
  return (
    <div className={`cv-template ${className}`} style={{ ...base, padding: 0, display: 'flex', ...style }}>
      <div style={{ width: '34%', background: '#BC181C', color: '#fff', padding: '40px 28px' }}>
